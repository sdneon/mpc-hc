...
static int opus_flush_resample(OpusStreamContext *s, int nb_samples)
{
    ...
    ret = swr_convert(s->swr,
                      ...
                      NULL, 0);
#elif CONFIG_AVRESAMPLE
    ret = avresample_convert(s->avr, (uint8_t**)s->cur_out, 0, nb_samples,
                             NULL, 0, 0);
#endif
    if (ret < 0)
        return ret;
...
#elif CONFIG_AVRESAMPLE
    ret = avresample_convert(s->avr, NULL, 0, 0, delayptr, 0,
...
#elif CONFIG_AVRESAMPLE
        samples = avresample_convert(s->avr, (uint8_t**)s->cur_out, 0,
                                     s->packet.frame_duration,
                                     (uint8_t**)s->silk_output, 0,
                                     samples);
#endif
        if (samples < 0) {
